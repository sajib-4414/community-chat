...
**To support both one to one and group chat**,
At first we created models such as room, roommembers, messages, users. For both private room(one to one chat) and group chat, i started to track using the roomMembers model. then i saw that in this approach i have to do an unncecessary join/aggregation between roomMembers and rooms for one to one chat. For one to one chat there is just two members, so I modifed the room model to keep the private chat members list. But for group chat members i will use the roomMembers model, so that I can scale and support unlimited users in a group chat.
...
### How to add geospatial location support?
* used opencage to get lat,long from a given address/zipcode.
* We must need a geospatial index before searching in db with location, such as finding users in a given radius.
so i created the index in the schema file.

User model
```
//userSchema declaration
....
location: {
        type: { 
            type: String, 
...
```
see the seedfile example how to create user with lat long
```
{
      ...
      "location": {
            "type": "Point",
            "coordinates": [-104.612159, 50.431355] 
        }
    }
```
# Frontnend related
...
what error boundary catch: which is not a compile time error such as syntax error, wrong tag,Errors that occur during the build process, like those reported by Babel or Vite, Errors Outside React Components: Errors in non-React code or configuration issues.
*what error boundary  catch*:  Rendering Errors: Errors that occur during the rendering of a component.
Lifecycle Methods: Errors in componentDidMount, componentDidUpdate, or other lifecycle methods.
Constructors: Errors that occur in the constructor of a component.
*One very Common example* is if there is no field in api response, we try to access, that time. this is a runtime error. or a variable got null we tried to access it during some logic in frontend.
I used react-error-boundary with a given fallback UI. i could have done it manually as well.
# Lessons
